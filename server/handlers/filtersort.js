...
exports.deleteFilterSortHandler = async function(ids, userId, next) {
	...
	for (let i = 0; i < ids.length; i++) {
		...
		const filtersort = await FilterSortModel.findById(environmentId).select('name user');
		if (!filtersort) return next(new ErrorResponse(`Environment not found with id of ${environmentId}`, 404));

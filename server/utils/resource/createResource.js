...
module.exports = async function createResource(model, userId, data) {
	...
	else if (modelName === 'Question') return await createQuestionHandler(userId, data, next);
	else if (modelName === 'Folder') return await createFolderHandler(userId, data, next);
	else if (modelName === 'Environment') return await createEnvironmentHandler(userId, data, next);

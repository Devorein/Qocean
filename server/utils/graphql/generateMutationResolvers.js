...
module.exports = function (
	resource,
	transformedSchema,
	MutationResolversTransformers
) {
	const pluralizedResource = pluralize(resource, 2);
	const capitalizedResource =
		resource.charAt(0).toUpperCase() + resource.substr(1);
	...
	if (mutations.create[0])
		MutationResolvers[`create${capitalizedResource}`] = async function (
			parent,
			{ data },
			ctx
		) {
	...
	if (mutations.update[0])
		MutationResolvers[`update${capitalizedResource}`] = async function (
			parent,
			{ data, id },
			ctx
		) {
	...
	if (mutations.update[1])
		MutationResolvers[
			`update${pluralizedcapitalizedResource}`
		] = async function (parent, { data, ids }, ctx) {
	...
	if (mutations.delete[0])
		MutationResolvers[`delete${capitalizedResource}`] = async function (
			parent,
			{ id },
			ctx
		) {
			return (
				await deleteResource(ctx[capitalizedResource], [id], ctx.user.id)
			)[0];
		};
	if (mutations.delete[1])
		MutationResolvers[
			`delete${pluralizedcapitalizedResource}`
		] = async function (parent, { ids }, ctx) {

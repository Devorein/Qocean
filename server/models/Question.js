...
QuestionSchema.statics.validateQuestion = async function (question) {
	...
	if (type === 'MCQ' || type === 'MS' || type === 'TF') {
		if (type === 'MCQ' || type === 'MS') {
			if (!question.options)
				return [false, 'Provide the options for the question'];
			if (question.options.length < 3)
				return [false, 'Provide atleast 3 options for the question'];
			if (question.options.length > 6)
				return [false, 'Options for the question cant be more than 6'];
			if (!Array.isArray(question.options))
				return [false, 'Options must be an array'];
			const containsDuplicate =
				new Set(question.options.map((option) => option.toString().trim()))
					.size !== question.options.length;
			if (containsDuplicate)
				return [false, 'There is duplicate options for the question'];
			if (question.answers.length > question.options.length)
				return [false, 'You provided more answer than options'];
			const containsDuplicateAnswer =
				new Set(question.answers.map((answer) => answer.toString().trim()))
					.size !== question.answers.length;
			if (containsDuplicateAnswer)
				return [false, 'There is duplicate answer for the question'];
			const isValidAnswer = question.answers.every(
				(answer) => parseInt(answer) >= 0 && parseInt(answer) <= 5
			);
			if (!isValidAnswer) return [false, 'Your answer is out of range'];
			if (type === 'MCQ')
				if (question.answers.length > 1)
					return [false, 'You provided   more answers than needed'];
		} else if (type === 'TF') {
			if (question.options.length >= 1)
				return [false, 'No need to provide the options'];
			if (question.answers.length > 1)
				return [false, 'You provided more answers than needed'];
			const isValidAnswer = question.answers.every(
				(answer) => parseInt(answer) >= 0 && parseInt(answer) <= 1
			);
			if (!isValidAnswer) return [false, 'Your answer is out of range'];
		}
		...
		if (question.answers.length >= 2)
			return [false, 'You provided too many answers'];
		if (question.answers[0].length <= 0)
			return [false, 'Provide atleast one answer for the question'];
		if (question.answers[0].length > 3)
			return [false, 'You provided too many answers'];
	} else if (type === 'FIB') {
		if (question.options.length >= 1)
			return [false, 'No need to provide the options'];
		...
		if (count !== question.answers.length)
			return [false, 'You provided incorrect number of answers'];
	}
	return [true];
};
...
QuestionSchema.methods.validateAnswer = async function (answers) {
	...
	if (type === 'MCQ' || type === 'TF') {
		...
		isCorrect =
			isCorrect && parseInt(answers[0]) === parseInt(this.answers[0][0]);
	} else if (type === 'MS') {
		...
		isCorrect =
			isCorrect &&
			!transformed.some((answer) => checkAgainst.indexOf(answer) === -1);
	} else if (type === 'Snippet')
		isCorrect = answers.length === 1 && typedChecker(this.answers[0], answers);
	else if (type === 'FIB') {
		isCorrect = answers.length === this.answers.length;
		isCorrect =
			isCorrect &&
			answers.every((answer, index) =>
				typedChecker(this.answers[index], answer)
			);
	} else if (type === 'FC') {
		isCorrect =
			answers.length !== 0 && answers.length <= this.answers[0].length;
		answers = answers.map((answer) => parseInt(answer));
		isCorrect =
			isCorrect && answers.every((answer) => answer >= 0 && answer <= 2);
	}
...
QuestionSchema.post('save', async function () {
	...
	if (this.isModified('time_allocated'))

...
casual.define('environment', function () {
	...
	const notification_timing =
		notification_timings[getRandomInt(0, notification_timings.length - 1)];
	...
	const envObj = {
		...
		question: {
			default_timing: time,
			default_type: ['FIB', 'Snippet', 'MCQ', 'MS', 'FC', 'TF'][
				getRandomInt(0, 5)
			],
		...
		default_create_landing: resources.splice(1)[
			getRandomInt(0, resources.length - 2)
		],
		notification_timing,
	...
	['explore', 'self', 'watchlist', 'play'].forEach((page) => {
		...
		target[`default_view`] = ['Table', 'List', 'Board', 'Gallery'][
			getRandomInt(0, 3)
		];
		...
		target[`default_landing`] = ((page) => {
			if (page === 'explore')
				...
				return resources.slice(1)[getRandomInt(0, resources.length - 2)];
			else if (page.match(/(watchlist|play)/))
				return ['Quiz', 'Folder'][getRandomInt(0, 1)];
			else return [];
		})(page);
	});
	return envObj;
